# ADR-0006: A dispatcher owns a slot by locking its position, and has no identity

## Status

Accepted (2026-09-17). Follows the decision taken first in the Rust
reference implementation, reached there through a discriminator pass: three
designs from different premises, three checked objections each, a synthesis.
Ported with the outbox.
...
1. **The division is a column.** A row carries its slot,
   ...
   nothing at run time can re-slice. `~slots` is read by `setup` only, which
   refuses a table cut otherwise.
...
3. **A dispatcher has no identity.** `dispatch outbox selection subscriber`;
   `run outbox ~clock ~loops ~shutdown selection subscriber`. Any number of
   loops in any number of processes share a selection through the locks; a
   process that dies releases its slot with its transaction, and the next
   poll of any survivor takes it. Rolling deployments are indistinguishable
   from steady state. The worker's identity was only ever needed to name a
   position row; a lock names it.
...
- `worker_id`, `num_workers`, `process_id` and `num_processes` go; `Loops.t`
  is the layout of one process. `positions` returns a position per slot,
  whose minimum is a watermark for the selection; `set_position` moves every
  slot.
- The position rows of a selection are created at its first contact, inside
  ...
  batch.
- Measured in the reference implementation on 200,000 rows, one statement,
  best of runs: with work at three quarters of the table 0.06 ms for one
  slot, 0.20 ms for sixteen, 0.26 ms for sixty-four; idle 0.03, 0.10 and
  0.11 ms. With `EXISTS` as the probe the one-slot case cost 2.4 ms, a
  bitmap over the tail.
...
- Observer events name the slot; `Slots` replaces `Workers` in the model,
  the state space unchanged. The model has `Rehash`, the assignment changing
  under the positions, and `OutboxRehash.cfg`, which must violate
...
- The inbox takes the same shape in ADR-0007.

## Alternatives rejected

**Partitions by configuration, positions per partition.** The loss
relocated to a runtime parameter of the fetch, which a process built with
another count re-sliced silently; and partitions in the API.

**Positions keyed by layout, `group:id/of`, a new layout starting from the
minimum of the last.** Keeps the worker identity literally. Redelivery on a
