...
(** Per-group state on a topic. At most one active wrapped callback (the [string -> unit]
    closure that decodes and invokes the typed user callback). *)
...
(** Adapter-internal types. Each carries enough state to do its own work without
    re-parameterizing on [broker] — the broker is captured in the closures returned by
    {!adapter}. *)
...
let adapter (broker : broker) : (module Ascetic_bus.Bus.Adapter) =
  (module struct
