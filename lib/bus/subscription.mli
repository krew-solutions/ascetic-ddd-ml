...
(** A subscription that {!cancel} detaches by running the function once, and then, every
    time it is called, waits on with [quiesce], which returns when no call of the handler
    is in flight; see {!Handling.quiesce}. Without one, [cancel] does not wait. *)
...
(** Detaches the handler, and returns when no call of it is in flight: it waits for a
    ...
    Idempotent: a second [cancel] detaches nothing, and waits like the first. *)
