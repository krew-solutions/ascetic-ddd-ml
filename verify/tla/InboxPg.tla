----------------------------- MODULE InboxPg -----------------------------
(***************************************************************************)
(* The inbox one level below Inbox.tla: the statements of `lib/inbox`,  *)
...
(* dependency's identity that a wait and a mark both take (ADR-0008).      *)
(*                                                                         *)
...
(*   WithLock    FALSE: no advisory lock -- the wait of ADR-0005 as it was  *)
(*               implemented, and the lost wake ADR-0008 fixed;            *)
(*   HeadInTake  TRUE: the head read in the take's statement, under a      *)
(*               snapshot older than the lock -- the take of ADR-0007 as   *)
(*               it was implemented;                                       *)
(*   WalkOn      TRUE: a dispatcher that set a head aside goes on to the   *)
(*               next head in the same transaction, holding the lock --    *)
(*               the first design of ADR-0008, refused for its lock cycle. *)
