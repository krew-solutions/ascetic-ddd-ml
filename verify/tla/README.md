...
(`lib/outbox/pg_outbox.ml`), and `check.sh` requires it to appear.
...
slots is an arbitrary function, so the sign of `hashtext` is out of scope
here and covered by a test instead. Who serves a slot is not
modelled either: a dispatcher is whoever holds the lock on the slot's
position, and has no other identity (ADR-0006).
...
Dependencies (ADR-0005). The walk looks at the head of the queue only. A head
...
- `InboxSplitWait.cfg`: the check of a dependency and the wait are two
  ...
  yet settled — the implementation before ADR-0008. TLC finds
  ...
  ADR-0008 is what makes the two steps one.

Failures and parking (ADR-0004). A subscriber that fails rolls its writes back
...
- `InboxPoisonNoParking.cfg`: the same with `MaxAttempts = 0`. TLC finds
  ...
  is the inbox before ADR-0004, and `check.sh` requires the violation.
...
level down, the statements of `lib/inbox` against PostgreSQL under
...
- `InboxPgNoLock.cfg`: `WithLock = FALSE`, the wait of ADR-0005 as it was
  ...
  ADR-0008 fixed, found by the model rather than by a test.
- `InboxPgHeadInTake.cfg`: `HeadInTake = TRUE`, the head read in the take's
  statement, the take of ADR-0007 as it was implemented. TLC finds
...
- `InboxPgWalkOn.cfg`: `WalkOn = TRUE`, a dispatcher that set a head aside
  ...
  ADR-0008. Two slots of two messages each, every head depending on a
...
tests attach the recorder of `lib/trace`, an observer of both the outbox
...
dependency, the lost wake of ADR-0008 as one query — and leaves the
...
before the take, for both libraries. An empty fetch and a handling are checks rather than steps: a
