...
with TLC. A model describes the participants and their steps — transactions,
dispatchers, workers, crashes, retries — and TLC explores every interleaving
on a small instance, looking for a state that breaks a property. What is
checked is the *protocol*, not the OCaml code: the gap between the two is
closed by tests, and later by trace validation, where test runs are checked
against the model.

The models are shared with the Rust port of these building blocks, which
implements the same protocol over the same SQL. They were written there
first and are kept identical here up to the paths they name.
...
(`lib/outbox/init.sql` explains it, `lib/outbox/outbox.ml` applies it), and
`check.sh` requires it to appear.
...
arbitrary function, so the sign of `hashtext` is out of scope here and
covered by the forty-key tests in `test/outbox` and `test/inbox` instead.

How the OCaml dispatcher maps onto the model: `Outbox.dispatch` is one
`Fetch`, a `Handle` per message and one `Ack`, all in one transaction.
`Outbox.Iter` acknowledges each message when the consumer asks for the next
one, but inside that same transaction, so the acknowledgements commit
together at the end of the batch exactly as the model's `Ack` does. A
subscriber returning `Error`, a raise, or `Iter.close` is `Crash`.
...
`EffectsIffProcessed` is the guarantee the inbox exists for, and the one an
earlier port's channel API broke by marking a message on delivery rather
than on completion. Here `Inbox.dispatch` and `Inbox.Iter` both mark inside
the transaction the subscriber ran in, after it returned.
...
handled on delivery and stores it afterwards, the shape of an earlier port's
channel API. TLC finds the loss on a single message: delivered, acknowledged,
the process dies, the store never happens. `check.sh` requires that
violation.

In this library the bridge is an outbox subscriber that calls
`Inbox.publish` and returns only once the row is stored. `Outbox.dispatch`
acknowledges after the subscriber and `Outbox.Iter` when the consumer asks
for the next message, so both are the `StoreBeforeAck` shape; a subscriber
that acknowledged first would have to be written on purpose.
